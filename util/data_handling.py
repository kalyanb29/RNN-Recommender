...
class DataHandler(object):
	...
	def __init__(self, dirname, extended_training_set=False, shuffle_training=False):
		...
		self.dirname = self._get_path()

		self.extended_training_set = extended_training_set
		if extended_training_set:
			self.training_set = SequenceGenerator(self.dirname+'data/data/train_set_sequences+',  shuffle=shuffle_training)
		else:
			self.training_set = SequenceGenerator(self.dirname+'data/data/train_set_sequences',shuffle=shuffle_training)
		self.validation_set = SequenceGenerator(self.dirname+'data/data/val_set_sequences')
		self.test_set = SequenceGenerator(self.dirname+'data/data/test_set_sequences')

		self._load_stats()

	def training_set_triplets(self):
		with open(self.dirname + 'data/data/train_set_triplets') as f:
			for line in f:
				line = line.split()
	...
	def item_rating(self):
		...
		if not hasattr(self.training_set, '_item_rating'):
			if os.path.isfile(self.dirname + 'data/data/training_set_item_rating.npy'):
				self.training_set._item_rating = np.load(self.dirname + 'data/data/training_set_item_rating.npy')
			else:
				...
				np.save(self.dirname + 'data/data/training_set_item_rating.npy', self.training_set._item_rating)
	...
	def item_popularity(self):
		...
		if not hasattr(self.training_set, '_item_pop'):
			...
			# else:
				...
				np.save(self.dirname + 'data/data/training_set_item_popularity.npy', self.training_set._item_pop)
	...
	def _load_stats(self):
		...
		'''
		with open(self.dirname+'data/data/stats', 'r') as f:
			...
			# self.latest_time = datetime.datetime.strptime(stats[5], '%Y-%m-%d') #.
