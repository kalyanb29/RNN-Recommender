...
def command_parser():
    ...
    parser.add_argument('-f', dest='filename', help='Input file',
                        default=os.getcwd() + '\data\data.csv', type=str)
    parser.add_argument('--columns',
                        help='Order of the columns in the file (eg: "uit"), u for user, i for item, t for timestamp. '
                             'If r is not present a default rating of 1 is given to all interaction. If t is not present interactions are assumed to be in chronological order. Extra columns are ignored. Default: uit',
...
def save_index_mapping(data, dirname):
    ...
    data['i'] = data['i_original'].cat.codes  # ex sunny windy .. -> 0 1 2..
